  ...
  return (
    <div style={{ maxWidth: 1100, margin: '0 auto', padding: '24px 16px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        ...
        <button onClick={syncTrending} disabled={fetching} style={{ background: '#EE4D2D', color: 'white', padding: '10px 20px', borderRadius: 8, border: 'none', cursor: 'pointer', fontWeight: 600, fontSize: 14 }}>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
        {[
          ...
          <button key={s.key} onClick={() => setSortBy(s.key)} style={{ padding: '6px 14px', borderRadius: 20, fontSize: 13, cursor: 'pointer', border: 'none', background: sortBy === s.key ? '#EE4D2D' : '#f0f0f0', color: sortBy === s.key ? 'white' : '#333', fontWeight: sortBy === s.key ? 600 : 400 }}>
            {s.label}
          </button>
        ))}
      </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: 12 }}>
          {products.map((p) => (
            <div key={p.id} className="card" style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              ...
              <a href={'/create?product_id=' + p.id} style={{ background: '#EE4D2D', color: 'white', padding: '8px', borderRadius: 8, fontSize: 13, textDecoration: 'none', textAlign: 'center', fontWeight: 600, marginTop: 'auto', display: 'block' }}>สร้างคลิป</a>
            </div>
          ))}
        </div>
      )}
